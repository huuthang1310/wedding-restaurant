<header class="header-fixed smaller">
    <div class="main-header bg-f">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-6 col-md-4 col-lg-3">
                    <a href="/home" class="logo"><img
                            src="img/logo-foody.png" alt="image"></a>
                </div>
                <div class="col-6 col-md-8 col-lg-7 hidden-md hidden-lg">
                    <nav id="holder_mn" class="menu-nav pull-right sub-menu-light">
                        <div class="menu-btn"></div>
                        <div class="menu-wrap">
                            <div class="menu-btn-close"></div>
                            <ul class="menu">
                                <li><a href="/home">home</a></li>
                                <li><a href="/about">about</a></li>
                                <li><a href="/menu">menu</a></li>
                                <li><a href="/reservations">reservations</a>
                                </li>
                                <li>
                                    <a href="/home">blog</a>
                                    <ul class="sub-menu">
                                        <li><a href="/blog">blog_list</a>
                                        </li>
                                        <li><a href="/single-blog">single_blog</a>
                                        </li>
                                    </ul>
                                </li>
                                <li>
                                    <a href="/home">pages</a>
                                    <ul class="sub-menu">
                                        <li><a href="/404-2">error_404</a>
                                        </li>
                                        <li><a href="/contact">contact</a>
                                        </li>
                                    </ul>
                                </li>
                            </ul>
                        </div>
                    </nav>
                </div>
                <div class="col-lg-2 d-none d-lg-block">
                    <a href="/home" class="btn float-right">Book a
                        table</a>
                </div>
            </div>
        </div>
    </div>
</header>