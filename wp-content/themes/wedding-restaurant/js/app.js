(function e(t, n, r) {
    ...
    4: [function(require, module, exports) {
        ...
        require("../../bower_components/owl.carousel/dist/owl.carousel");
        require("../../bower_components/jquery-flipster/dist/jquery.flipster.min");

        require('./libs/header');
        require('./libs/incentives');
        require('./libs/slider');
        require('./libs/serviceSlider');
        require('./libs/loader');
        require('./libs/menu');
        require('./posts');

        require('./libs/remodalVideo');

        console.log('remodal viewo');
        ...
        var setUpIncentives = (function() {
            function activeIncentive(incentiveComponent) {
                var title = incentiveComponent.attr('data-title');
                var content = incentiveComponent.attr('data-content');
                var description = incentiveComponent.attr('data-description');
                var incentiveElement = incentiveComponent.closest('.section-incentive').find('.incentive-data');
                incentiveComponent.closest('.incentives').find('.incentive').removeClass('active');
                incentiveElement.find('h2').html(title);
                incentiveElement.find('p').html(content);
                incentiveElement.find('i').html(description);
            }

            $('.incentive').hover(function() {
                activeIncentive($(this));
            });

            $('.incentive a').on('click', function(e) {
                if (detectmobHome()) {
                    var incentiveComponent = $(this).closest('.incentive');
                    if (!incentiveComponent.hasClass('active')) {
                        e.preventDefault();
                    }
                    activeIncentive(incentiveComponent);
                    incentiveComponent.addClass('active');
                }
            });
        })
        ...
        setUpIncentives();
        setUpBackToTop();
    }, { "../../bower_components/bootstrap-sass/assets/javascripts/bootstrap.min": 1, "../../bower_components/jquery-flipster/dist/jquery.flipster.min": 2, "../../bower_components/owl.carousel/dist/owl.carousel": 3, "./libs/header": 5, "./libs/incentives": 6, "./libs/loader": 7, "./libs/menu": 8, "./libs/remodalVideo": 9, "./libs/serviceSlider": 10, "./libs/slider": 11, "./posts": 12 }],
    5: [function(require, module, exports) {
        $("#header").on('click', '.btn-toggle', function(e) {
            e.preventDefault();
            var icon = this;
            var header = $(this).closest('#header')
            header.toggleClass('showNavigation');
            if (header.hasClass('showNavigation')) {
                console.log('sow');
                $(icon).find('.fa')[0].className = 'fa fa-close';
            } else {
                $(icon).find('.fa')[0].className = 'fa fa-bars';
            }
        })

    }, {}],
    6: [function(require, module, exports) {
        var galleryGrid = $('.incentiveLists');
        var galleryGridContainer = $('.incentiveLists__container');
        var btnPrev = galleryGrid.find('.btn-prev');
        var btnNext = galleryGrid.find('.btn-next');
        var galleryGridCarousel = galleryGridContainer.owlCarousel({
            items: 4,
            margin: 15,
            autoplay: false,
        ...
        btnPrev.click(function() {
            galleryGridCarousel.trigger('prev.owl.carousel')
        });
        btnNext.click(function() {
            galleryGridCarousel.trigger('next.owl.carousel')
        })
    ...
    8: [function(require, module, exports) {
        var menu = $('#navigationSpecial');
        var buttonToggle = menu.find('.navigationToggle__button');
        ...
        buttonToggle.on('click', function() {
            if (menu.hasClass('active')) {
                ...
                logo.removeClass("fadeOutUp animated");
                logo.addClass("fadeInDown animated");
            } else {
                $(this).html('<i class="fa fa-close"></i>');
                menu.addClass('active');
                logo.addClass("fadeOutUp animated");
            }
        });

        $("#fixedMenu").affix({
            offset: {
                top: logo.offset().top + 100
            }
        });
    ...
    9: [function(require, module, exports) {
        //var setUpModalVideoYoutube = (function () {
        ...
        //            height: '300',
        //            width: '100%',
        //            videoId: $('#youtube-player').attr('data-youtube-id'),
        //            playerVars: {
        //                autoplay: false,
        //                rel: 0
        //            }
        ...
        //        var videoContainerTop = $('.section-video-container').offset().top;
        //        if (scrollTop >= videoContainerTop - 300 && scrollTop <= videoContainerTop + 300 && !videoPlayed) {
        //            videoPlayed = true;
        //            player.playVideo();
        //        }
        //        if (scrollTop > videoContainerTop + 300 && videoPlayed){
        //            videoPlayed = false;
        //            player.pauseVideo();
        //        }
        //    });
        //
        //    $('.page-template-page-home #header-navbar').affix({
        //        offset: {
        //            top: $('.navbar-fake').offset().top + 100
        //        }
        //    });
        //
        //    $('a.icon-menu-fixed').on('click', function (e) {
        //        e.preventDefault();
        //        $('.navbar-fake').toggleClass('active');
        //        $('.header-navbar').toggleClass('display');
        //        $('.header-sliders').toggleClass('hide-box')
        //    });
        //});

        $(document).on('opening', '.remodal-youtube-video', function() {
            var videoUrl = $(this).find('.youtube-video').attr('data-url');
            $(this).find('.youtube-video').html(
                '<iframe width="100%" height="400" src="' + videoUrl + '" frameborder="0" allowfullscreen autoplay></iframe>'
            )
        });

        $(document).on('closing', '.remodal-youtube-video', function(e) {
            $(this).find('.youtube-video').html('');
        });
    ...
    10: [function(require, module, exports) {
        var services = $('.servicesContainerSliders');
        var serviceGridContainer = services.find('.servicesContainer');
        var btnPrev = services.find('.btn-prev');
        var btnNext = services.find('.btn-next');

        var serviceCarousel = serviceGridContainer.owlCarousel({
            margin: 30,
            autoplay: false,
            autoplayTimeout: 3000,
            loop: true,
    ...
    11: [function(require, module, exports) {
        var descriptionSlider = $('.descriptionCarouselContainer').find('.descriptionCarousel');
        var descriptionCarousel;

        if (descriptionSlider.length > 0) {
            descriptionCarousel = descriptionSlider.owlCarousel({
                loop: false,
                dots: false,
                items: 1
            })
        ...
        function itemSwitchParent(index) {
            if (descriptionCarousel) {
                descriptionCarousel.trigger('to.owl.carousel', index);
            }
        }

        var SliderFlipter = function() {
            var slidersContainer = $(".slidersContainer");
            var onItemSwitch = function(element) {
                itemSwitchParent($(element).attr('data-index'));
                setTimeout(function() {
                    PlaceholderThumbnail(flipterSlider, $(element).attr('data-index'));
                }, 50);
            }

            var autoplaySpeed = parseInt(slidersContainer.attr('data-autoplaySpeed'));
            var flipterSlider = slidersContainer.flipster({
                spacing: -0.5,
                loop: true,
                start: 'center',
                touch: true,
                scrollwheel: false,
                autoplay: !isNaN(autoplaySpeed) && autoplaySpeed ? autoplaySpeed : false,
                onItemSwitch: onItemSwitch
            });

            slidersContainer.on('click', '.nav-prev', function() {
                flipterSlider.flipster('prev');
            });

            slidersContainer.on('click', '.nav-next', function() {
                flipterSlider.flipster('next');
            });

            //if(autoplaySpeed > 0 && !isNaN(autoplaySpeed)){
            ...
            return flipterSlider;
        }

        const PlaceholderThumbnail = function(flipterSlider, dataIndex) {
            if (!dataIndex) {
                var currentIndex = $(flipterSlider).find('.flipster__item--current').attr('data-index');
            } else {
                currentIndex = dataIndex;
            }
            $('.slidersSlickThumbnailContainer').find('.sliderThumbnail__item').removeClass('active');
            $('.slidersSlickThumbnailContainer').find('.sliderThumbnail__item[data-index=' + currentIndex + ']').addClass('active');
        }

        const SliderThumbnail = function(flipterSlider) {
            var slidersSlickThumbnail = $('.slidersSlickThumbnailContainer');
            var btnPrev = slidersSlickThumbnail.find('.btn-prev');
            var btnNext = slidersSlickThumbnail.find('.btn-next');

            var sliderCarousel = slidersSlickThumbnail.find('.sliderThumbnail').owlCarousel({
                margin: 10,
                autoplay: false,
                autoplayTimeout: 3000,
                loop: true,
            ...
            slidersSlickThumbnail.on('click', '.sliderThumbnail__item', function() {
                var sliderIndex = parseInt($(this).attr('data-index'));
                flipterSlider.flipster('jump', sliderIndex);
                PlaceholderThumbnail(flipterSlider, sliderIndex);
            })
            ...
            PlaceholderThumbnail(flipterSlider)
        }

        var flipterSlider = SliderFlipter();
        SliderThumbnail(flipterSlider);
